...
export function getLatestRpgScratchpadSnapshot(maxLookback = 80) {
    return getLatestRpgScratchpadSnapshotFromMessages(chat, maxLookback);
}
...
export function evaluateRpgContentTrigger(settings, stmbData, highestProcessed, currentLastMessage, buffer) {
    ...
    const scratchpadTrigger = evaluateScratchpadTriggerFromMessages(chat, sceneStart, safeEnd);
    const scratchpadSignal = scratchpadTrigger?.scratchpadSignal || null;
    ...
    const duplicateRequired = isDuplicateRequiredScratchpadTrigger(stmbData, scratchpadTrigger);
    ...
    if (scratchpadTrigger && !scratchpadTrigger.candidateOnly) {
        return {
    ...
    return {
        ...
        scratchpadSignal,
        memoryScope: scratchpadSignal?.scope || '',
        temporalAnchor: scratchpadSignal?.temporalAnchor || '',
    };
}
